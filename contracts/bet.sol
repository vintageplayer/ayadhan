...
contract Bet {
    ...
    struct MoveSigns {
        bytes32 previousMessage;
        bytes32 previousSignature;
        bytes32 currentMessage;
        bytes32 currentSignature;
    ...
    mapping(uint=>MoveSigns) latestGameSign;
    ...
    function setSignedMessage(uint gameId, bytes32 _previousMessage, bytes32 _previousSignature, bytes32 _currentMessage, bytes32 _currentSignature)
    public
    validGame(gameId)
    ...
    isSignedGame(gameId)
    {
        latestGameSign[gameId].previousMessage = _previousMessage;
        latestGameSign[gameId].previousSignature = _previousSignature;
        latestGameSign[gameId].currentMessage = _currentMessage;
        latestGameSign[gameId].currentSignature = _currentSignature;
        latestGameSign[gameId].nextMessageDeadline = block.timestamp + 86400000;
    }
